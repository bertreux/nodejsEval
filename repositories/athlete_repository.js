...
const insert_new_athlete = async (data) => {
    const db = await db_connection();
    try {
        const [ results ] = await db.query('INSERT INTO jeux_olympiques.athlete VALUE(NULL, :nom, :prenom, :age, :sexe, :pays_id)',
            data)
        return results;
    } catch (error) {
        return error;
...
const update_athlete_by_id = async (id, data) => {
    ...
    data.id_athlete = id;
    try {
        const [ results ] = await db.query('UPDATE jeux_olympiques.athlete SET athlete.nom = :nom, athlete.prenom = :prenom, athlete.age = :age, athlete.sexe = :sexe, athlete.pays_id = :pays_id where athlete.id_athlete = :id_athlete',
            data)
        return results;
    } catch (error) {
        return error;
