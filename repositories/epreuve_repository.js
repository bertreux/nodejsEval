...
const insert_new_epreuve = async (data) => {
    const db = await db_connection();
    try {
        const [ results ] = await db.query('INSERT INTO jeux_olympiques.epreuve VALUE(NULL, :Nom, :sport_id, :athlete_id)',
            data)
        return results;
    } catch (error) {
        return error;
...
const update_epreuve_by_id = async (id, data) => {
    ...
    data.id_epreuve = id;
    try {
        const [ results ] = await db.query('UPDATE jeux_olympiques.epreuve SET epreuve.Nom = :Nom, epreuve.sport_id = :sport_id, epreuve.athlete_id = :athlete_id where epreuve.id_epreuve = :id_epreuve',
            data)
        return results;
    } catch (error) {
        return error;
